  ...
  foreach ($r in @($Manifest.Results | Where-Object Workstream -eq $w)) {
   ...
   if ($r.RowCount -gt 0) {
    ...
    if ($preview.Count -gt 0) {
     [void]$h.Append('<details><summary>View first 100 rows; CSV contains all collected rows</summary><div class="scroll"><table><thead><tr>')
     $cols = @($preview[0].PSObject.Properties.Name)
     foreach ($c in $cols) { [void]$h.Append("<th>$(ConvertTo-HtmlText $c)</th>") }
     [void]$h.Append('</tr></thead><tbody>')
     foreach ($row in $preview) {
      [void]$h.Append('<tr>')
      foreach ($c in $cols) { [void]$h.Append("<td>$(ConvertTo-HtmlText $row.$c)</td>") }
      [void]$h.Append('</tr>')
     }
